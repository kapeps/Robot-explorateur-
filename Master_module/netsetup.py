############
# netsetup.py for Micropython on ESP32
#
# This module reads a list of network SSIDs from a network.json file
...
#
# If it is connected to the Internet, it can optionally setup a timer to get the time 
# from pool.ntp.org every hour and set the RTC time and date.
#
# © Frédéric Boulanger <frederic.softdev@gmail.com>
# 2019-08-30 -- 2020-05-16
...
print("Trying to connect to known wifi AP")
...
if len(netprio) > 0 :
  ...
  for net in netprio : # Try networks in priority order
    ...
    if ssid in netnames :
      ...
      for _ in range(5) : # Try 5 times, waiting 1 second for the connection
        utime.sleep(1)  
        if wlan.isconnected() :
          print("Success!")
          break
...
if len(usocket.getaddrinfo('pool.ntp.org', 123)) > 0 :
  ...
  # May not be a good idea if you plan to use Timer(-1) for something else
  # ntp_timer = Timer(-1)
